<div class="messages">
		...
		<?php if($record->read == 0) : ?>
      	...
      	<?php $i++; ?>
      			...
      			<!-- <div class="control-group">
				    <div class="controls">
				      	<label class="checkbox">
				        	<input id='checkbox' type="checkbox" name="msg[]" value="<?php echo $record->msg_id; ?>">
				     	</label>
				  	</div>
				</div> -->
      		</td>
      		...
      		<td>
      			...
      			<?php if($record->read == 0) : ?>
      				<strong><a title='<?php echo $record->subject; ?>' href="<?php echo site_url('messages/messages/read_mail') ?><?php echo '/'. $record->msg_id ?>"><?php echo substr($record->subject,0, 40);  ?></a></strong>
      			<?php else :?>
      				<a title='<?php echo $record->subject; ?>'  href="<?php echo site_url('messages/messages/read_mail') ?><?php echo '/'. $record->msg_id ?>"><?php echo substr($record->subject,0, 40); ?></a>
      			<?php endif; ?>
      		</td>
      		<td>
	...
	<!-- <input type="submit" name="reply" id='reply' class="btn btn-small btn-inverse" value="Απάντηση" />
