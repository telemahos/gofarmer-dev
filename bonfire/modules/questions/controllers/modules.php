...
class modules extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Questions.Modules.View');
		$this->load->model('questions_model', null, true);
		$this->lang->load('questions');
		
		Template::set_block('sub_nav', 'modules/_sub_nav');
	}
	...
	*/
	...
	{
		$this->auth->restrict('Questions.Modules.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_questions())
			{
				...
				Template::redirect(SITE_AREA .'/modules/questions');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/modules/questions');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Questions.Modules.Edit');
		...
		{
			$this->auth->restrict('Questions.Modules.Delete');
